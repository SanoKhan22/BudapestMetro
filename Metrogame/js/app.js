...
(function() {
  ...
  /**
   * Handle round transition to new line
   */
  function handleRoundTransition() {
    ...
    // Highlight new line's endpoints
    const currentLine = GameState.getCurrentLine();
    if (currentLine) {
      GridView.highlightEndpoints(currentLine.getEndpoints());
    }
  ...
  /**
   * Handle skip card - draw next card without building segment
   */
  function handleSkipCard() {
    const currentCard = GameState.getCurrentCard();
    
    // Only allow skip if a card has been drawn
    if (!currentCard) {
  ...
  function handleEndRound() {
    ...
    if (!GameState.isGameComplete()) {
      ...
      // Clear score display and show ready for new round
